...
function trendChart(series,keys,colors,unit){if(!series.length)return '<div class="trend-empty">当前范围暂无可绘制数据，完成一轮模型调用后会显示趋势。</div>';const width=360,height=184,left=44,right=12,top=12,bottom=30,plotW=width-left-right,plotH=height-top-bottom,rows=series.length,scale=unit==='%'?100:1,max=Math.max(1,...series.flatMap(point=>keys.map(key=>(Number(point[key])||0)*scale))),xAt=index=>left+(rows===1?plotW/2:index*plotW/(rows-1)),yAt=value=>top+plotH-(value/max)*plotH,points=key=>series.map((point,index)=>`${xAt(index)},${yAt((Number(point[key])||0)*scale)}`).join(' '),dots=key=>series.map((point,index)=>`<circle cx="${xAt(index)}" cy="${yAt((Number(point[key])||0)*scale)}" r="3" fill="${colors[keys.indexOf(key)]}"/>`).join(''),first=series[0]?.time?new Date(series[0].time).toLocaleTimeString():'' ,last=series.at(-1)?.time?new Date(series.at(-1).time).toLocaleTimeString():'';return `<div class="trend-chart-wrap"><div class="trend-y-label"><span>${max.toLocaleString()} ${unit}</span><span>数值</span><span>0 ${unit}</span></div><svg class="trend-chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-label="横轴为时间，纵轴单位为 ${unit}"><line x1="${left}" y1="${top}" x2="${left}" y2="${height-bottom}" stroke="#dfe6ef"/><line x1="${left}" y1="${height-bottom}" x2="${width-right}" y2="${height-bottom}" stroke="#dfe6ef"/>${keys.map((key,index)=>`<polyline points="${points(key)}" fill="none" stroke="${colors[index]}" stroke-width="2.5" vector-effect="non-scaling-stroke"/>${dots(key)}`).join('')}<text x="${left}" y="${height-7}" class="trend-axis-text">${first}</text><text x="${width-right}" y="${height-7}" text-anchor="end" class="trend-axis-text">${last}</text></svg></div>`}
...
function renderBoard(){
  ...
  addTrendEnhancements(tokenSeries);bindEvaluationDay(days);
