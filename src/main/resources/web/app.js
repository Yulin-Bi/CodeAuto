...
function renderBoard(){
  ...
  $('#board').innerHTML=`<div class="board-toolbar"><div><b>运行评估</b><small>${scope} · 指标只统计选定范围</small></div><div class="board-scope"><button class="${state.boardMode==='session'?'active':''}" data-board-scope="session">当前会话</button><button class="${state.boardMode==='project'?'active':''}" data-board-scope="project">当前项目</button></div></div><div class="board-grid">${[['turns','完成轮次'],['toolCalls','工具调用'],['toolErrorRate','工具错误率'],['contextTokens','上下文 tokens']].map(([k,l])=>`<div class="board-card"><b>${k==='toolErrorRate'?toolRateText:(m[k]||0).toLocaleString()}</b><span>${l}${k==='toolErrorRate'?` · ${toolErrors}/${toolCalls}`:''}</span></div>`).join('')}</div><div class="board-chart"><div class="board-chart-head"><b>上下文与运行趋势</b><span>横轴：上下文事件序号 · 纵轴：tokens · ${evaluation.seriesAvailable?'已读取真实事件':'当前范围暂无上下文事件'}</span></div><div class="chart-axis"><span>${max.toLocaleString()} tokens</span><span>0 tokens</span></div><svg viewBox="0 0 220 54" preserveAspectRatio="none" aria-label="横轴为上下文事件序号，纵轴单位为 tokens"><line x1="20" y1="6" x2="220" y2="6" stroke="#edf2f8"/><line x1="20" y1="49" x2="220" y2="49" stroke="#dfe6ef"/><polyline points="${poly}" fill="none" stroke="#246bfd" stroke-width="2" vector-effect="non-scaling-stroke"/></svg><div class="chart-legend"><span><i class="legend-line blue"></i>上下文 tokens</span><span><i class="legend-line green"></i>缓存/压缩事件</span><span><i class="legend-line amber"></i>工具错误率 ${(toolRate*100).toFixed(1)}%</span></div></div><details class="reflection-panel" open><summary><b>反思与 Bullet</b><span>${reflectionRows.length?'已读取本地记录':(reflections.scopeNote||'当前范围暂无记录')}</span></summary><div class="reflection-scope"><button class="${state.reflectionMode==='session'?'active':''}" data-reflection-scope="session">当前会话</button><button class="${state.reflectionMode==='project'?'active':''}" data-reflection-scope="project">当前项目</button></div><div class="reflection-list">${reflectionRows.length?reflectionRows.map(item=>`<button class="memory-row" data-memory-id="${esc(item.id)}" data-memory-type="${item.bulletId?'bullet':'reflection'}"><small>${item.bulletId?'Bullet':'反思'} · ${esc(item.updatedAt||'')}</small><b>${esc(item.title||'未命名')}</b></button>`).join(''):`<div class="reflection-empty">${esc(reflections.scopeNote||'没有找到对应范围的反思或 Bullet 文件')}</div>`}</div></details>`
