...
function trendChart(series,keys,colors,unit){const width=360,height=184,left=54,right=12,top=12,bottom=25,plotW=width-left-right,plotH=height-top-bottom,rows=series.length||1,max=Math.max(1,...series.flatMap(point=>keys.map(key=>Number(point[key])||0))),points=key=>series.map((point,index)=>{const value=Number(point[key])||0;return `${left+(value/max)*plotW},${top+(rows===1?plotH/2:index*plotH/(rows-1))}`}).join(' '),first=series[0]?.time?new Date(series[0].time).toLocaleTimeString():'' ,last=series.at(-1)?.time?new Date(series.at(-1).time).toLocaleTimeString():'';return `<div class="trend-chart-wrap"><div class="trend-y-label"><span>${last}</span><span>时间</span><span>${first}</span></div><svg class="trend-chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-label="纵轴为时间，横轴单位为 ${unit}"><line x1="${left}" y1="${top}" x2="${left}" y2="${height-bottom}" stroke="#dfe6ef"/><line x1="${left}" y1="${height-bottom}" x2="${width-right}" y2="${height-bottom}" stroke="#dfe6ef"/>${keys.map((key,index)=>`<polyline points="${points(key)}" fill="none" stroke="${colors[index]}" stroke-width="2.5" vector-effect="non-scaling-stroke"/>`).join('')}<text x="${left}" y="${height-6}" class="trend-axis-text">0 ${unit}</text><text x="${width-right}" y="${height-6}" text-anchor="end" class="trend-axis-text">${max.toLocaleString()} ${unit}</text></svg></div>`}
