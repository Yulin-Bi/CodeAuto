const state={sessions:[],active:null,metrics:{},evaluation:null,reflections:null,events:[],transcript:[],contextWindow:0,todoMode:'current',railView:'overview',boardMode:'session',reflectionMode:'session',evaluationDay:'',evaluationTimeScale:'auto',expandedSessions:new Set(),treeReady:false,gitAvailable:false,rootChangedFiles:0,worktrees:[],worktreeGraph:null,pendingApprovals:[]}
...
document.body.insertAdjacentHTML('beforeend',`<dialog id="permission-dialog" class="permission-dialog"><div class="permission-shell"><div class="permission-kicker"><span class="permission-pulse"></span><span>需要你的批准</span></div><h2 id="permission-title">Agent 请求执行操作</h2><p id="permission-summary"></p><div class="permission-scope"><span id="permission-kind">操作</span><code id="permission-scope"></code></div><label class="permission-auto"><span><b>后续同类请求</b><small>只影响当前这一轮任务，结束后自动恢复询问。</small></span><select id="permission-auto-policy"><option value="ASK">每次都询问</option><option value="EDITS">自动允许文件修改</option><option value="ALL">自动允许所有请求</option></select></label><div class="permission-guide"><b>怎么选？</b><span><strong>仅这次</strong>：只执行当前请求。</span><span><strong>本轮允许</strong>：本轮同类请求不再打断。</span><span><strong>始终允许</strong>：以后也自动允许，请谨慎使用。</span></div><label class="permission-feedback"><span>拒绝原因（可选）</span><textarea id="permission-feedback" placeholder="告诉 Agent 应该如何调整"></textarea></label><div id="permission-error" class="fork-error"></div><div class="permission-actions"><button type="button" class="permission-deny" data-permission-action="deny">拒绝</button><button type="button" class="permission-secondary" data-permission-decision="ALLOW_ONCE">仅这次</button><button type="button" class="permission-secondary" data-permission-decision="ALLOW_TURN">本轮允许</button><button type="button" class="permission-primary" data-permission-decision="ALLOW_ALWAYS">始终允许</button></div></div></dialog>`)
...
function assignBranchColors(branches=[]){const names=[...new Set(branches.map(String))].sort((a,b)=>a.localeCompare(b));branchColors.clear();let next=0;for(const name of names){const clean=name.replace(/^refs\/heads\//,'');if(clean==='main'||clean==='master')branchColors.set(name,'#2878d0');else if(clean==='develop'||clean.endsWith('/develop'))branchColors.set(name,'#8b5cf6');else{branchColors.set(name,branchPalette[next%branchPalette.length]);next++}}}
function branchColor(branch){const name=String(branch||'main').replace(/^refs\/heads\//,'');if(name==='main'||name==='master')return '#2878d0';if(name==='develop'||name.endsWith('/develop'))return '#8b5cf6';if(!branchColors.has(name)){const used=new Set(branchColors.values());let color=branchPalette.find(item=>!used.has(item))||branchPalette[branchColors.size%branchPalette.length];branchColors.set(name,color)}return branchColors.get(name)||branchPalette[0]}
...
function renderVerticalWorktreeGraph(){
  const graph=state.worktreeGraph||{},nodes=graph.nodes||[],branches=Object.entries(graph.branches||{}).sort(([a],[b])=>{const rank=name=>name==='main'||name==='master'?0:name==='develop'||name.endsWith('/develop')?1:2;return rank(a)-rank(b)||a.localeCompare(b)});
  assignBranchColors(branches.map(([branch])=>branch));
  const visible=nodes.slice(0,100),index=new Map(visible.map((node,i)=>[node.hash,i])),laneByHash=new Map(),laneExpect=[],freeLanes=[];
  visible.forEach(node=>{let lane=laneByHash.get(node.hash);if(lane===undefined){lane=laneExpect.indexOf(node.hash);if(lane<0)lane=freeLanes.shift()??laneExpect.length;}
    laneByHash.set(node.hash,lane);const parents=node.parents||[];laneExpect[lane]=parents[0]||null;
    for(let i=1;i<parents.length;i++){const parent=parents[i],existing=laneExpect.indexOf(parent);if(existing<0){const extra=freeLanes.shift()??laneExpect.length;laneExpect[extra]=parent;}}
    for(let i=0;i<laneExpect.length;i++)if(laneExpect[i]===node.hash&&i!==lane){laneExpect[i]=null;freeLanes.push(i)}
  });
  const laneCount=Math.max(1,laneExpect.length,branches.length),laneGap=21,width=Math.max(240,72+laneCount*laneGap),height=Math.max(160,visible.length*34+35),x=lane=>24+lane*laneGap,y=i=>22+i*34,laneBranches=branches.map(([branch])=>branch),selectedLane=laneBranches.indexOf(selectedGraphBranch);
...
function trendChart(series,keys,colors,unit){if(!series.length)return '<div class="trend-empty">当前日期暂无记录</div>';return `<div class="trend-chart-wrap"><canvas class="trend-canvas" data-chart-unit="${unit}" aria-label="横轴为时间，纵轴单位为 ${unit}"></canvas></div>`}
function renderCharts(tokenSeries,errorSeries){
  if(typeof Chart==='undefined')return;
  document.querySelectorAll('.trend-canvas').forEach(canvas=>{if(canvas._chart)canvas._chart.destroy()});
  const times=[...tokenSeries,...errorSeries].map(p=>Date.parse(p.time)).filter(Number.isFinite);let start=Date.parse(`${state.evaluationDay}T00:00:00`),end=start+86400000;
  if(state.evaluationTimeScale==='auto'&&times.length){const lo=Math.min(...times),hi=Math.max(...times),padding=Math.max(15*60*1000,(hi-lo)*.08);start=lo-padding;end=hi+padding;if(end-start<30*60*1000){const mid=(start+end)/2;start=mid-15*60*1000;end=mid+15*60*1000}}
  const span=end-start,tickFormat=span<=6*3600000?{hour:'2-digit',minute:'2-digit'}:span<=2*86400000?{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}:{month:'numeric',day:'numeric'};
  const common={responsive:true,maintainAspectRatio:false,animation:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{callbacks:{title(items){return items[0]?new Date(items[0].parsed.x).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):''}}}},scales:{x:{type:'linear',min:start,max:end,grid:{color:'#edf1f5'},ticks:{maxTicksLimit:7,autoSkip:true,callback:v=>new Date(v).toLocaleString('zh-CN',tickFormat)}},y:{beginAtZero:true,grid:{color:'#edf1f5'},ticks:{callback:v=>Number(v).toLocaleString()}}}};
  const tokenCanvas=document.querySelector('.trend-canvas[data-chart-unit="tokens"]');
  if(tokenCanvas)tokenCanvas._chart=new Chart(tokenCanvas,{type:'line',data:{datasets:[['inputTokens','输入 tokens','#246bfd'],['outputTokens','输出 tokens','#2c9b72'],['contextTokens','上下文总 tokens','#8b5cf6'],['cacheReadTokens','缓存命中 tokens','#d28a20']].map(([key,label,color])=>({label,borderColor:color,backgroundColor:color,pointRadius:2,borderWidth:2,tension:.25,data:tokenSeries.map(p=>({x:Date.parse(p.time),y:Number(key==='cacheReadTokens'?(p.cacheReadTokens??p.cachedTokens??p.promptCacheHitTokens):p[key])||0}))}))},options:common});
  const errorCanvas=document.querySelector('.trend-canvas[data-chart-unit="%"]');
  if(errorCanvas)errorCanvas._chart=new Chart(errorCanvas,{type:'line',data:{datasets:[{label:'工具正确率',borderColor:'#d28a20',backgroundColor:'#d28a20',pointRadius:2,borderWidth:2,tension:.25,data:errorSeries.map(p=>({x:Date.parse(p.time),y:(Number(p.rate)||0)*100}))}]},options:{...common,scales:{...common.scales,y:{...common.scales.y,max:100,ticks:{callback:v=>`${v}%`}}}}});
}
function addTrendEnhancements(tokenSeries){const charts=document.querySelectorAll('.trend-chart-wrap');charts.forEach((wrap,index)=>{const svg=wrap.querySelector('.trend-chart');if(!svg)return;const first=index===0?tokenSeries[0]?.time:null,last=index===0?tokenSeries.at(-1)?.time:null;if(!wrap.querySelector('.trend-x-label')){const labels=document.createElement('div');labels.className='trend-x-label';labels.innerHTML=`<span>${first?new Date(first).toLocaleTimeString():''}</span><span>时间</span><span>${last?new Date(last).toLocaleTimeString():''}</span>`;wrap.appendChild(labels)}if(index===0&&tokenSeries.some(point=>Number(point.cacheReadTokens)>0)){const values=tokenSeries.map(point=>Number(point.cacheReadTokens)||0),max=Math.max(1,...tokenSeries.flatMap(point=>['inputTokens','outputTokens','contextTokens','cacheReadTokens'].map(key=>Number(point[key])||0))),width=360,height=184,left=44,right=12,top=12,bottom=30,plotW=width-left-right,plotH=height-top-bottom,points=values.map((value,i)=>`${left+(values.length===1?plotW/2:i*plotW/(values.length-1))},${top+plotH-(value/max)*plotH}`).join(' '),line=document.createElementNS('http://www.w3.org/2000/svg','polyline');line.setAttribute('points',points);line.setAttribute('fill','none');line.setAttribute('stroke','#d28a20');line.setAttribute('stroke-width','2.5');line.setAttribute('vector-effect','non-scaling-stroke');line.setAttribute('class','cache-hit-line');svg.appendChild(line)}});const legend=document.querySelector('.board-chart .chart-legend');if(legend&&!legend.querySelector('.cache-hit-legend')){const item=document.createElement('span');item.className='cache-hit-legend';item.innerHTML='<i class="legend-line amber"></i>缓存命中 tokens';legend.appendChild(item)}}
function dayKey(time){return time?new Date(time).toISOString().slice(0,10):''}
function availableDays(...series){return [...new Set(series.flat().map(point=>dayKey(point.time)).filter(Boolean))].sort().reverse()}
function dayLabel(day){if(!day)return '';const date=new Date(`${day}T00:00:00`);return date.toLocaleDateString('zh-CN',{year:'numeric',month:'long',day:'numeric',weekday:'short'})}
function downsampleSeries(series,limit=80){if(series.length<=limit)return series;const step=(series.length-1)/(limit-1),result=[];for(let i=0;i<limit;i++)result.push(series[Math.round(i*step)]);return result}
function alignTrendToTime(series,keys,unit,chartIndex=0){const chart=document.querySelectorAll('.trend-chart')[chartIndex];if(!chart||!series.length)return;const width=360,height=184,left=44,right=12,top=12,bottom=30,plotW=width-left-right,plotH=height-top-bottom,scale=unit==='%'?100:1,max=Math.max(1,...series.flatMap(point=>keys.map(key=>(Number(point[key])||0)*scale))),dayStart=Date.parse(`${state.evaluationDay}T00:00:00`),dayEnd=dayStart+86400000,xAt=point=>left+Math.max(0,Math.min(1,(Date.parse(point.time)-dayStart)/(dayEnd-dayStart)))*plotW,yAt=value=>top+plotH-(value/max)*plotH;chart.querySelectorAll('polyline').forEach((line,lineIndex)=>{const key=keys[lineIndex]||'cacheReadTokens';const points=series.map(point=>`${xAt(point)},${yAt((Number(point[key])||0)*scale)}`).join(' ');line.setAttribute('points',points)})}
function bindEvaluationDay(days){const toolbar=$('.board-toolbar');if(!toolbar)return;let select=toolbar.querySelector('.evaluation-day');if(!select){select=document.createElement('select');select.className='evaluation-day';toolbar.appendChild(select)}select.innerHTML=days.map(day=>`<option value="${day}" ${day===state.evaluationDay?'selected':''}>${dayLabel(day)}</option>`).join('');select.onchange=()=>{state.evaluationDay=select.value;boardUi.signature='';renderBoard()}}
function bindEvaluationTimeScale(){const scope=$('.board-scope');if(!scope)return;let select=scope.querySelector('.evaluation-time-scale');if(!select){select=document.createElement('select');select.className='evaluation-time-scale';select.setAttribute('aria-label','时间范围');select.innerHTML='<option value="auto">自适应时间</option><option value="day">全天 00:00–24:00</option>';scope.prepend(select)}select.value=state.evaluationTimeScale;select.onchange=()=>{state.evaluationTimeScale=select.value;boardUi.signature='';renderBoard()}}
function renderBoard(){
  const session=state.sessions.find(x=>x.id===state.active),evaluation=state.evaluation||{},m=evaluation.metrics||{},scope=state.boardMode==='project'?'当前项目':'当前会话',allTokenSeries=evaluation.tokenSeries||[],allErrorSeries=evaluation.toolSuccessSeries||[],cacheRate=Number(m.cacheHitRate||0),experienceRate=Number(m.experienceHitRate||0),toolRate=Number(m.toolSuccessRate||0),toolRateText=`${(toolRate*100).toFixed(1)}%`,toolErrors=Number(m.toolErrors||0),toolCalls=Number(m.toolCalls||0)
  const days=availableDays(allTokenSeries,allErrorSeries);if(!state.evaluationDay||!days.includes(state.evaluationDay))state.evaluationDay=days[0]||'';const tokenSeries=downsampleSeries(allTokenSeries.filter(point=>dayKey(point.time)===state.evaluationDay)),errorSeries=downsampleSeries(allErrorSeries.filter(point=>dayKey(point.time)===state.evaluationDay)),reflections=state.reflections||{reflections:[],bullets:[]},reflectionScope=state.reflectionMode==='project'?'当前项目':'当前会话',reflectionRows=(reflections.reflections||[]).slice(0,12)
  ...
  const metricCards=[['turns','完成轮次'],['toolCalls','工具调用'],['toolSuccessRate','工具正确率'],['cacheHitRate','缓存命中率'],['experienceHitRate','经验命中率'],['contextTokens','上下文 tokens']];
  const cards=metricCards.map(([k,l])=>{const value=k==='toolSuccessRate'?toolRateText:k==='cacheHitRate'?`${(cacheRate*100).toFixed(1)}%`:k==='experienceHitRate'?`${(experienceRate*100).toFixed(1)}%`:(m[k]||0).toLocaleString();return `<div class="board-card"><b>${value}</b><span>${l}${k==='toolSuccessRate'?` · ${toolCalls-toolErrors}/${toolCalls}`:''}</span></div>`}).join('');
  $('#board').innerHTML=`<div class="board-toolbar"><div><b>运行评估</b><small>${scope} · 指标只统计选定范围，数据持久化保存</small></div><div class="board-scope"><button class="${state.boardMode==='session'?'active':''}" data-board-scope="session">当前会话</button><button class="${state.boardMode==='project'?'active':''}" data-board-scope="project">当前项目</button></div></div><div class="board-grid">${cards}</div><div class="board-chart"><div class="board-chart-head"><b>Token 使用趋势</b><span>横轴：时间 · 纵轴：tokens · 输入、输出、上下文总量</span></div>${trendChart(tokenSeries,['inputTokens','outputTokens','contextTokens'],['#246bfd','#2c9b72','#8b5cf6'],'tokens')}<div class="chart-legend"><span><i class="legend-line blue"></i>输入 tokens</span><span><i class="legend-line green"></i>输出 tokens</span><span><i class="legend-line purple"></i>上下文总 tokens</span></div></div><div class="board-chart error-chart"><div class="board-chart-head"><b>工具正确率趋势</b><span>横轴：时间 · 纵轴：正确率 · ${toolCalls-toolErrors}/${toolCalls} 次</span></div>${trendChart(errorSeries,['rate'],['#d28a20'],'%')}<div class="chart-legend"><span><i class="legend-line amber"></i>工具正确率</span></div></div><details class="reflection-panel" open><summary><b>反思与 Bullet</b><span>${reflectionRows.length?'已读取本地记录':(reflections.scopeNote||'当前范围暂无记录')}</span></summary><div class="reflection-scope"><button class="${state.reflectionMode==='session'?'active':''}" data-reflection-scope="session">当前会话</button><button class="${state.reflectionMode==='project'?'active':''}" data-reflection-scope="project">当前项目</button></div><div class="reflection-list">${reflectionRows.length?reflectionRows.map(item=>`<button class="memory-row" data-memory-id="${esc(item.id)}" data-memory-type="${item.bulletId?'bullet':'reflection'}"><small>${item.bulletId?'Bullet':'反思'} · ${esc(item.updatedAt||'')}</small><b>${esc(item.title||'未命名')}</b></button>`).join(''):`<div class="reflection-empty">${esc(reflections.scopeNote||'没有找到对应范围的反思或 Bullet 文件')}</div>`}</div></details>`
  renderCharts(tokenSeries,errorSeries);bindEvaluationDay(days);bindEvaluationTimeScale();
  ...
  document.querySelectorAll('.trend-x-label').forEach(label=>{label.innerHTML='<span>00:00</span><span>06:00</span><span>12:00</span><span>18:00</span><span>24:00</span>'});document.querySelectorAll('[data-board-scope]').forEach(button=>button.onclick=async()=>{state.boardMode=button.dataset.boardScope;await loadBoardData();renderBoard()})
