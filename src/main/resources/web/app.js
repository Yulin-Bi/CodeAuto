...
function renderBoard(){
  const session=state.sessions.find(x=>x.id===state.active),evaluation=state.evaluation||{},m=evaluation.metrics||{},scope=state.boardMode==='project'?'当前项目':'当前会话',tokenSeries=evaluation.tokenSeries||[],errorSeries=evaluation.toolSuccessSeries||[],cacheRate=Number(m.cacheHitRate||0),experienceRate=Number(m.experienceHitRate||0),toolRate=Number(m.toolSuccessRate||0),toolRateText=`${(toolRate*100).toFixed(1)}%`,toolErrors=Number(m.toolErrors||0),toolCalls=Number(m.toolCalls||0)
  ...
  addTrendEnhancements(tokenSeries);
