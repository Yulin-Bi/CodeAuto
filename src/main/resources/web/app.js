...
function renderCharts(tokenSeries,errorSeries){
  ...
  const start=Date.parse(`${state.evaluationDay}T00:00:00`),end=start+86400000;
  const common={responsive:true,maintainAspectRatio:false,animation:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{callbacks:{title(items){return items[0]?new Date(items[0].parsed.x).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):''}}}},scales:{x:{type:'linear',min:start,max:end,grid:{color:'#edf1f5'},ticks:{maxTicksLimit:5,callback:v=>new Date(v).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',hour12:false})}},y:{beginAtZero:true,grid:{color:'#edf1f5'},ticks:{callback:v=>Number(v).toLocaleString()}}}};
...
function renderBoard(){
  ...
  renderCharts(tokenSeries,errorSeries);bindEvaluationDay(days);
