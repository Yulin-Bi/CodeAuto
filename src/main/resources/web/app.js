...
function trendChart(series,keys,colors,unit){if(!series.length)return '<div class="trend-empty">当前范围暂无可绘制数据，完成一轮模型调用后会显示趋势。</div>';const width=360,height=184,left=54,right=12,top=12,bottom=25,plotW=width-left-right,plotH=height-top-bottom,rows=series.length,scale=unit==='%'?100:1,max=Math.max(1,...series.flatMap(point=>keys.map(key=>(Number(point[key])||0)*scale))),points=key=>series.map((point,index)=>{const value=(Number(point[key])||0)*scale;return `${left+(value/max)*plotW},${top+(rows===1?plotH/2:index*plotH/(rows-1))}`}).join(' '),dots=key=>series.map((point,index)=>{const value=(Number(point[key])||0)*scale;return `<circle cx="${left+(value/max)*plotW}" cy="${top+(rows===1?plotH/2:index*plotH/(rows-1))}" r="3" fill="${colors[keys.indexOf(key)]}"/>`}).join(''),first=series[0]?.time?new Date(series[0].time).toLocaleTimeString():'' ,last=series.at(-1)?.time?new Date(series.at(-1).time).toLocaleTimeString():'';return `<div class="trend-chart-wrap"><div class="trend-y-label"><span>${last}</span><span>时间</span><span>${first}</span></div><svg class="trend-chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-label="纵轴为时间，横轴单位为 ${unit}"><line x1="${left}" y1="${top}" x2="${left}" y2="${height-bottom}" stroke="#dfe6ef"/><line x1="${left}" y1="${height-bottom}" x2="${width-right}" y2="${height-bottom}" stroke="#dfe6ef"/>${keys.map((key,index)=>`<polyline points="${points(key)}" fill="none" stroke="${colors[index]}" stroke-width="2.5" vector-effect="non-scaling-stroke"/>${dots(key)}`).join('')}<text x="${left}" y="${height-6}" class="trend-axis-text">0 ${unit}</text><text x="${width-right}" y="${height-6}" text-anchor="end" class="trend-axis-text">${max.toLocaleString()} ${unit}</text></svg></div>`}
function renderBoard(){
  const session=state.sessions.find(x=>x.id===state.active),evaluation=state.evaluation||{},m=evaluation.metrics||{},scope=state.boardMode==='project'?'当前项目':'当前会话',tokenSeries=evaluation.tokenSeries||[],errorSeries=evaluation.toolErrorSeries||[],cacheRate=Number(m.cacheHitRate||0),experienceRate=Number(m.experienceHitRate||0),toolRate=Number(m.toolErrorRate||0),toolRateText=`${(toolRate*100).toFixed(1)}%`,toolErrors=Number(m.toolErrors||0),toolCalls=Number(m.toolCalls||0)
  ...
  const metricCards=[['turns','完成轮次'],['toolCalls','工具调用'],['toolErrorRate','工具错误率'],['cacheHitRate','缓存命中率'],['experienceHitRate','经验命中率'],['contextTokens','上下文 tokens']];
  const cards=metricCards.map(([k,l])=>{const value=k==='toolErrorRate'?toolRateText:k==='cacheHitRate'?`${(cacheRate*100).toFixed(1)}%`:k==='experienceHitRate'?`${(experienceRate*100).toFixed(1)}%`:(m[k]||0).toLocaleString();return `<div class="board-card"><b>${value}</b><span>${l}${k==='toolErrorRate'?` · ${toolErrors}/${toolCalls}`:''}</span></div>`}).join('');
  $('#board').innerHTML=`<div class="board-toolbar"><div><b>运行评估</b><small>${scope} · 指标只统计选定范围</small></div><div class="board-scope"><button class="${state.boardMode==='session'?'active':''}" data-board-scope="session">当前会话</button><button class="${state.boardMode==='project'?'active':''}" data-board-scope="project">当前项目</button></div></div><div class="board-grid">${cards}</div><div class="board-chart"><div class="board-chart-head"><b>Token 使用趋势</b><span>纵轴：时间 · 横轴：tokens · 输入、输出、上下文总量</span></div>${trendChart(tokenSeries,['inputTokens','outputTokens','contextTokens'],['#246bfd','#2c9b72','#8b5cf6'],'tokens')}<div class="chart-legend"><span><i class="legend-line blue"></i>输入 tokens</span><span><i class="legend-line green"></i>输出 tokens</span><span><i class="legend-line purple"></i>上下文总 tokens</span></div></div><div class="board-chart error-chart"><div class="board-chart-head"><b>工具错误率趋势</b><span>纵轴：时间 · 横轴：错误率 · ${toolErrors}/${toolCalls} 次</span></div>${trendChart(errorSeries,['rate'],['#d28a20'],'%')}<div class="chart-legend"><span><i class="legend-line amber"></i>工具错误率</span></div></div><details class="reflection-panel" open><summary><b>反思与 Bullet</b><span>${reflectionRows.length?'已读取本地记录':(reflections.scopeNote||'当前范围暂无记录')}</span></summary><div class="reflection-scope"><button class="${state.reflectionMode==='session'?'active':''}" data-reflection-scope="session">当前会话</button><button class="${state.reflectionMode==='project'?'active':''}" data-reflection-scope="project">当前项目</button></div><div class="reflection-list">${reflectionRows.length?reflectionRows.map(item=>`<button class="memory-row" data-memory-id="${esc(item.id)}" data-memory-type="${item.bulletId?'bullet':'reflection'}"><small>${item.bulletId?'Bullet':'反思'} · ${esc(item.updatedAt||'')}</small><b>${esc(item.title||'未命名')}</b></button>`).join(''):`<div class="reflection-empty">${esc(reflections.scopeNote||'没有找到对应范围的反思或 Bullet 文件')}</div>`}</div></details>`
