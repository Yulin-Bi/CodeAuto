...
const iconSvg=name=>`<svg class="ui-icon" aria-hidden="true"><use href="/icons.svg#${name}"></use></svg>`
...
const branchPalette=['#f26b38','#df3e7b','#8b5cf6','#2878d0','#0f9f9a','#2f9d61','#c98913','#d84d58']
function branchColor(branch){const name=String(branch||'main').replace(/^refs\/heads\//,'');if(name==='main'||name==='master')return '#2878d0';if(name==='develop'||name.endsWith('/develop'))return '#8b5cf6';let hash=0;for(const char of name)hash=(hash*31+char.charCodeAt(0))>>>0;return branchPalette[hash%branchPalette.length]}
...
function renderVerticalWorktreeGraph(){
  ...
  const visible=nodes.slice(0,100),index=new Map(visible.map((node,i)=>[node.hash,i])),laneCount=Math.max(1,branches.length),width=Math.max(240,90+laneCount*30),height=Math.max(160,visible.length*34+35),x=lane=>28+lane*30,y=i=>22+i*34,laneBranches=branches.map(([branch])=>branch),selectedLane=laneBranches.indexOf(selectedGraphBranch);
  ...
  const nodeSvg=visible.map((node,i)=>{const lane=laneByHash.get(node.hash)||0,branch=laneBranches[lane],color=branchColor(branch),active=selectedLane<0||lane===selectedLane;return `<g class="graph-commit ${active?'':'dimmed'}" style="--graph-color:${color}" tabindex="0" role="button" data-graph-branch="${esc(branch)}" data-commit-detail="${esc(node.subject||'(无提交信息)')} · ${esc(node.author||'未知作者')} · ${esc(node.hash.slice(0,8))}"><circle cx="${x(lane)}" cy="${y(i)}" r="4"/><text x="${x(lane)+13}" y="${y(i)+4}">${esc(node.subject||'(无提交信息)')}</text></g>`}).join('');
...
function renderWorktrees(){
  const rail=$('#right-rail'),railTop=rail?.scrollTop||0,graphScroll=$('.worktree-graph-scroll'),graphTop=graphScroll?.scrollTop||worktreeGraphScrollTop;worktreeGraphScrollTop=graphTop;const current=$('#rail-worktree-current'),list=$('#rail-worktree-list'),session=state.sessions.find(item=>item.id===state.active)
...
const stream=new EventSource('/api/events');stream.addEventListener('agent_event',e=>{try{const event=JSON.parse(e.data);if(!state.active)state.active=event.sessionId;if(event.sessionId===state.active)addEvent(event);refresh().catch(()=>{})}catch{}});stream.onopen=()=>{$('#connection').textContent='实时连接'};stream.onerror=()=>{$('#connection').textContent='连接重试中';$('#connection').classList.add('busy')}
